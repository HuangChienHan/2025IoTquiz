...
import { Eye, X, Check, Loader2, Flag } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useRouter } from "next/navigation";

export default function WrongQuestionsList() {
    ...
    const fetchQuestions = async () => {
        try {
            const res = await fetch("/api/questions/wrong");
            if (res.ok) {
        ...
        <div className="max-w-6xl mx-auto py-8 px-4">
            <header className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold text-slate-800">歷史錯題</h1>
                </div>
                <div className="flex items-center gap-2">
                    <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full font-bold text-sm">共 {questions.length} 題</span>
                    <button
                        onClick={() => router.push("/quiz/wrong-review")}
                        className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-md transition-colors flex items-center gap-2"
                    >
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                <h2 className="text-xl font-bold text-slate-800">題目詳情 (歷史錯題)</h2>
